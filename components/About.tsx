        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{opacity:0,x:-30}} animate={inView?{opacity:1,x:0}:{}} transition={{duration:0.8,delay:0.2}}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'16px',
              ...
              <p>
                <span style={{color:'#fff',fontWeight:500}}>Senior Frontend Developer</span> with{' '}
              ...
              <p>
                My journey started in <span style={{color:'#FCD34D'}}>web design</span> — converting PSD/Figma to pixel-perfect HTML —
                which gave me a designer's eye combined with engineering precision.
              </p>
              <p>
                Currently at <span style={{color:'#fff',fontWeight:500}}>Indai Technologies</span>, building high-traffic
                applications achieving Lighthouse scores of{' '}
                <span style={{color:'#6EE7B7',fontFamily:'JetBrains Mono'}}>85–95+</span> and
                reducing load times by up to{' '}
                <span style={{color:'#6EE7B7',fontFamily:'JetBrains Mono'}}>40%</span>.
              </p>
              <p>
                <span style={{color:'#A78BFA',fontWeight:500}}>BCA graduate</span> from Vinayaka Mission University
                with a JavaScript ES6 certification (Unstop, 2026).
              </p>
            </div>

            <div style={{ marginTop:'24px', display:'flex', flexWrap:'wrap', gap:'8px' }}>
              {['React.js','Next.js','TypeScript','Tailwind CSS','REST APIs','Lighthouse'].map(t=>(
            ...
            <div style={{ marginTop:'28px', display:'flex', gap:'14px', flexWrap:'wrap' }}>
              ...
              <a href="https://linkedin.com/in/jainulkhan" target="_blank" rel="noopener noreferrer"
                style={{ color:'rgba(0,198,255,0.7)', fontSize:'13px', fontFamily:'JetBrains Mono',
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px' }}>
            {cards.map(({icon:Icon,title,desc,c,bg,border,extra},i)=>(
              ...
              >
                <div style={{ width:38, height:38, borderRadius:'10px', background:bg,
                  border:`1px solid ${border}`, display:'flex', alignItems:'center',
                  justifyContent:'center', marginBottom:'12px' }}>
                  <Icon size={19} className={c||extra||''} style={!c?{color:'#6EE7B7'}:{}}/>
                </div>
                <h3 style={{ fontFamily:'Space Grotesk', fontWeight:600, color:'#fff',
                  fontSize:'15px', marginBottom:'6px' }}>{title}</h3>
